...
ChangeLOG:
[2015-01-29] v1.5.0129
  - Previous modifications for Windows 10 before build 9926 was useless, since
    start menu merged into another different application
    ClassName:Windows.UI.Core.CoreWindow
    WindowText:Windows Shell Experience Host
    WindowRect to Screen:[L:0, T:0, W:1920, H:1040]
    WindowRect to Parent:[L:0, T:0, W:1920, H:1040]
    ClientRect:[L:0, T:0, W:1920, H:1040]
    WindowsClassStyle:[0x00000003]
    WindowsClassStyle = CS_HREDRAW | CS_VREDRAW
    ThreadId:[0x00002bec], ProcessId:[0x00003fc4]
    STYLE:[0x94000000], EXSTYLE:[0x94000000]
    nStyle = WS_OVERLAPPED | WS_POPUP | WS_VISIBLE | WS_CLIPSIBLINGS
    nExStyle = WS_EX_TOPMOST | WS_EX_LEFT | WS_EX_LTRREADING | WS_EX_RIGHTSCROLLBAR | 0x00200000L
    TextColor:[0x00000000], BackColor:[0x00ffffff]

     So we must be aware, that it would change for the next upcoming version

    After so many tries, it seems that this new menu/screen doesn't support layered windows background
    it is just like the tiles, including the background, so if I enable thickframe and caption
    those areas will be the only affected.

    But it can bee changed its opacity.

[2014-10-02] v1.4.1002
  - Adding support for Windows 10 TP (Tech Preview) x64 EN
  - We need to use both options:
    Class = DV2ControlHost Title = Start Menu (this last one might change according to language)
    Sub classes:
      - DesktopProgramsMFU
      - Desktop NSCHost
      - Desktop More Programs Pane
      - Desktop OpenBox Host
      - Desktop Grid (this is important, it is what contains the Tiles)
      - DesktopDestinationList
      - DesktopStartHeaderPane
      - DesktopStartPlacementPane
    Class = ImmersiveLauncher (old startmenu)

  - Added a way to change the created picture using the Menu rectsize
[2014-08-07] v1.4.0807
  - Respect Taskbar Area
    procedure ShowTaskbar // unconsidered dut to modernui apps which hides taskbar when calling startscreen
    so it looks ugly

[2014-08-03] v1.4.0803
  - Make it aware of screen changes
    FixToggle, FixResizeStartScreen()
[2013-10-20] v1.4.20
  - Get average color of an image algorithm in order to maybe apply the custom scheme and dwm aero colorization
  - Autolorize windows according to picture in start screen background
    Using GetAVGColor and change event on imgview321
[2013-10-18] v1.4.18
TODO  - Resolution changes (either by multimonitor or normal) saved in a variable so we
    save wasteful queries to the actual state
[2013-10-10] v1.4.10
...
type
  TForm1 = class(TForm)
    ...
    chkTaskbar: TCheckBox;  procedure FormCreate(Sender: TObject);
    ...
    procedure ImgView321Change(Sender: TObject);
...
var
  ...
  IsWin10TP: boolean = false;
  IsWin10TP2: boolean = false;
  //gIF
  gif: TGIFImage;
  gir: TGIFRenderer;
  ...
  MenuClass: string; //This will contain eithr StartScreen or StartMenu class, only on Windows 10
  ...
  'StartScreen不透明',
  'タイル不透明',
  'StartScreenを見るにはここをクリック',
  '私はそれを愛する',
  'このツールのような場合は、多分あなたはあなたのサポートを示したいと思います',
  'スライドショー用の画像を選択',
  '保存',
  'キャンセル',
  '言語',
  ...
  '当てる',
  'デフォルト',
  'For Windows 8/8.1 32bit/64bit.'#13#13'このツールは、あなたがあなたStartScreen背景画像をカスタマイズするのに役立ちます。'+
  #13#13'© Victor Alberto Gil Hancco Laquita a.k.a. vhanla'#13'http://codigobit.net'#13#13'Windows 8 ® は、マイクロソフト社の登録商標です。'
  );
...
const
 ...
 //igual que el anterior ^
  Themes10TP: array[1..19] of integer = (
 20001,
 20201,
 20301,
 20401,
...
function IsWindows10TP:boolean;
var
  r: TRegistry;
  CurrentVersion, CurrentBuild: string;
begin
  ...
  try
    ...
    try
      ...
      if r.OpenKeyReadOnly('Software\Microsoft\Windows NT\CurrentVersion') then
      begin
        CurrentVersion := r.ReadString('CurrentVersion'); //6.4
        CurrentBuild := r.ReadString('CurrentBuildNumber'); //9841
        if (CurrentVersion = '6.4')and(StrToInt(CurrentBuild) >= 9841)  then
        begin
          IsWin81 := False;  //desconocemos como windows 8.1
          IsWin81RTM := False; //desconocemos como windows 8.1
          IsWin10TP := True; //utilizará el mismo theme de windows 81 rtm
          Result := True;
        end;
...
function IsWindows10TP2:boolean;
var
  r: TRegistry;
  CurrentVersion, CurrentBuild: string;
begin
  ...
  try
    ...
    try
      ...
      if r.OpenKeyReadOnly('Software\Microsoft\Windows NT\CurrentVersion') then
      begin
        CurrentVersion := r.ReadString('CurrentVersion'); //6.4 TP anterior was bigger
        CurrentBuild := r.ReadString('CurrentBuildNumber'); //9841 TP anterior
        if (CurrentVersion = '6.3')and(StrToInt(CurrentBuild) >= 9926)  then
        begin
          IsWin81 := False;  //desconocemos como windows 8.1
          IsWin81RTM := False; //desconocemos como windows 8.1
          IsWin10TP := False; //utilizará el mismo theme de windows 81 rtm
          IsWin10TP2 := True; //totalmente distinto a los anteriores
          Result := True;
        end;
...
var
  ...
  r: TRect;
begin
  StartScreenHWND:=FindWindow(PChar(MenuClass),nil);
...
(*
Get Average color procedure
*)

procedure GetAvgColor(ASource: TBitmap32; var DColor: TColor);
type
...
var
  ...
  r,g,b: cardinal;
begin
  if not ASource.Empty then
  begin
    //lets first resize the picture to a 32x32 size
    pixelated:= TBitmap32.Create;
    try
      pixelated.SetSize(32,32);
      asource.DrawTo(pixelated,pixelated.BoundsRect, asource.BoundsRect);
      //lets analyze it
      Row := PColorBGRA(@pixelated.Bits[0]);
      r:=0;g:=0;b:=0;

      for I := 0 to pixelated.Height-1 do
      begin
        for J := 0 to pixelated.Width-1 do
        begin
          //$AABBGGRR
          r := r + Row.r;
          g := g + Row.g;
      ...
      //lets divide by 32*32 pixels
      r:=trunc(r/1024);
      g:=trunc(g/1024);
      b:=trunc(b/1024);
      Dcolor := (b shl 16)+(g shl 8)+r;
...
begin
  StartScreenHWND:=FindWindow(PChar(MenuClass),nil);
//  StartScreenHWND:=FindWindow('DV2ControlHost',nil);

  if IsWin10TP2 then
  begin
    StartScreenHWND := FindWindow(PChar(MenuClass), 'Windows Shell Experience Host');
  end;
...
try
  ...
  begin
    ...
    ini.WriteBool('Extra','Colorize',False);
  end
  else
  begin
    ...
    begin
      ...
      begin
        ...
        chkColorize.Checked:=ini.ReadBool('Extra','Colorize',False);
...
try
  ...
  ini.WriteBool('Extra','Colorize',chkColorize.Checked);
finally
  ini.UpdateFile;
  ini.Free;
end;
...
begin
  ...
  if FindWindow(PChar(MenuClass),nil)<>0 then
    begin
      SetWindowLong(
        FindWindow(PChar(MenuClass),nil),
        GWL_EXSTYLE,
        GetWindowLong(FindWindow(PChar(MenuClass),nil), GWL_EXSTYLE) and not WS_EX_LAYERED);
      RedrawWindow(FindWindow(PChar(MenuClass),nil),nil,0,RDW_ERASE or RDW_INVALIDATE or RDW_FRAME or RDW_ALLCHILDREN);

     SetWindowLong(FindWindow(PChar(MenuClass),nil), GWL_EXSTYLE, GetWindowLong(FindWindow(PChar(MenuClass),nil), GWL_EXSTYLE) Or WS_EX_LAYERED);
     SetLayeredWindowAttributes(FindWindow(PChar(MenuClass),nil),0,TrackBar1.Position, LWA_ALPHA);
    end;
...
begin
  ...
  //change button caption
    btnLoadPicture.Caption := Japanese[1];
    btnPreview.Caption := Japanese[2];
    btnHide.Caption := Japanese[3];
    btnExit.Caption := Japanese[4];
    pageControl1.Pages[0].Caption := Japanese[5];
    pageControl1.Pages[1].Caption := Japanese[6];
    chkDesktopWallpaper.Caption := Japanese[7];
    chkGaussianBlur.Caption := Japanese[8];
    btnSlideshow.Caption := Japanese[9];
    chkSlideshow.Caption := Japanese[10];
    chkRandom.Caption := Japanese[11];
    lblInterval.Caption := Japanese[12];
    chkStartup.Caption := Japanese[13];
    lblStartScreenOpacityTB.Caption := Japanese[14];
    lblTilesOpacityTB.Caption := Japanese[15];
    img320x240.Hint:=Japanese[16];
    lblLike.Caption := Japanese[17];
    imgLike.Left := lblLike.Left+lblLike.Width + 10;
    lblSupport.Caption := Japanese[18];
    frmMain.Caption := Japanese[19];
    frmMain.btnSave.Caption := Japanese[20];
    frmMain.btnCancel.Caption := Japanese[21];
    lblLanguage.Caption := Japanese[22];
    lblRowsNumber.Caption := Japanese[23];
    btnTilesRowApply.Caption := Japanese[24];
    btnTilesRowRestore.Caption := Japanese[25];
...
(* Let's generate our base picture *)
  ...
  try
    ...
    //src.SetSize(screen.Width,screen.Height);
    if MenuClass = 'DV2ControlHost' then
    begin
      GetWindowRect(FindWindow('DV2ControlHost',nil), startMenuRect);
      src.SetSize(startMenuRect.Width, startMenuRect.Height);
    end
    else
    src.SetSize(ScreenWidth, ScreenHeight);
...
//  ShowMessage(IntToStr(RealScreenWidthResolution));
  ...
  IsWin10TP := IsWindows10TP; //Es igual a Win81RTM
  IsWin10TP2 := IsWindows10TP2; // distinto a todos los anteriores

  if IsWin10TP then
  begin
    if FindWindow('ImmersiveLauncher', nil) <> 0 then
    begin
      MenuClass := 'ImmersiveLauncher';
    end
    else
    begin
      //we are in the start menu mode
      MenuClass := 'DV2ControlHost';
    end;
  end
  else
  begin
  ...
  if IsWin10TP2 then
  begin
//    if FindWindow('Windows.UI.Core.CoreWindow','Windows Shell Experience Host') <> 0 then
    if FindWindow('Windows.UI.Core.CoreWindow','Inicio') <> 0 then
    begin
      MenuClass := 'Windows.UI.Core.CoreWindow';
    end;
  end;

  //disable things that we won't use on win8.1
  if IsWin81 or IsWin10TP or IsWin10TP2 then
  begin
    //see if dpi is on
...
(*  if not DirectoryExists(gTempDir) then
  ...
  else if IsWin81RTM or IsWin10TP or IsWin10Tp2 then
  begin
...
begin
  ...
  else if IsWin81RTM or IsWin10TP then
  begin
...
begin
  if IsWin81RTM or IsWin10TP then
  begin
...
procedure TForm1.ImgView321Change(Sender: TObject);
var
  cl: tcolor;
begin
  //
  ...
  begin
    getavgcolor(imgview321.bitmap,cl);
    setcompositioncolor(cl);
  end;
end;
...
begin
  ShellExecute(Handle, 'OPEN',pchar('http://gifeditor.net/gifs/startscreen'),nil,nil,SW_SHOWNORMAL);
end;
...
var
  ...
  r: TRect;
begin
  wnd := FindWindow(PChar(MenuClass),nil);
...
// FIXES THE START SCREEN window to let it show the taskbar
// only if taskbar is not hidden
...
begin
  ...
  begin
    ...
    and (Shell_TrayWndRect.Right = ScreenWidth)
...
*)
procedure FixResizedStartScreen;
var
  h,t: HWND;
  r,s: TRect;
begin
  h :=  FindWindow(PChar(MenuClass),nil);
  if h <> 0 then
  begin
    GetWindowRect(h, r);
    t := FindWindowEx(h,0,'DirectUIHWND',nil);
    if t <> 0 then begin
      GetWindowRect(t,s);
      if (s.Right-s.Left <> r.Right-r.Left ) or (s.Bottom-s.Top <> r.Bottom-r.Top) then
      begin
        SetWindowPos(h, HWND_TOP, 0,0, r.Right-r.Left-1, r.Bottom-r.Top-1, SWP_SHOWWINDOW);
        SetWindowPos(h, HWND_TOP, 0,0, r.Right-r.Left, r.Bottom-r.Top, SWP_SHOWWINDOW);
      end;
    end;
...
begin
  ...
  if (GetForegroundWindow = FindWindow(PChar(MenuClass),nil))
//  and not chkSlideShow.Enabled
  then
  begin
    btnPreviewClick(self);
    //fixsize
    if FixSizeToggle then
    begin
      FixResizedStartScreen;
      FixSizeToggle := False;
    end;
  end
  else
  begin
    //disable startscreen fix size
    FixSizeToggle := True;
  end;
end;
